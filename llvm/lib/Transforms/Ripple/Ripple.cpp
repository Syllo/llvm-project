...
#if 0 // WIP - general case with structure
    NDLoadStoreAttr Attr(AddressSeries, StrideDims);
    if (Attr.hasNoStructure()) {
      // No particular structure - generate a scatter
      return irBuilder.CreateMaskedGather(vecTy, NewAddr, Load->getAligned());
    } else {
      // General case
      int rank = ToShape.rank();
      std::string & FunctionName = ndFunctionName("nd_load", rank, Attr);
      FunctionType * FunctionTy;
      // TODO: compute type
      FunctionCallee & FC = Mod.getOrInsertFunction(FunctionName, FunctionTy);
      Function * Callee = cast<Function> FC.getCallee();
      // Generate a call to the n-d load function
    }
#else
...
Value *NDLoadStoreFactory::genStore(StoreInst *Store, Value *Val,
  ...
  if (Contiguous) {
    ...
    // TODO: general case with structure
#if 0
    int NElements = ToShape.flatShape();
    Type *VecTy = VectorType::get(ElementTy, NElements, /*scalable=*/false);
#endif
...
Ripple::DimSize
Ripple::blockSize(RippleIntrinsicIndex rIdx,
                  std::map<PEIdentifier, TensorShape::Shape> &vectorShapes) {
  auto vectorShapeIt = vectorShapes.find(rIdx.first);
  if (vectorShapeIt != vectorShapes.end()) {
    auto &shape = vectorShapeIt->second;
    if (rIdx.second < shape.size())
      return shape[rIdx.second];
  }
  return 1;
}
...
void Ripple::padToTargetSIMDWidth() {
  ...
  auto GetPaddedV =
      [this, &GetPaddedLength](
          ...
          std::map<std::pair<Instruction *, unsigned>, Value *> InstToPadded)
      -> Value * {
    if (!V->getType()->isVectorTy()) {
  ...
  std::stack<Instruction *> InstructionsToRemove;

  std::map<PHINode *, PHINode *> OldPhiToNewPhi;
  std::vector<Instruction *> InstsThatMustBePaddedRPOT;
  ...
  for (auto *I : InstsThatMustBePaddedRPOT) {
    if (auto *ExtractElementI = dyn_cast<ExtractElementInst>(I)) {
      ...
      InstructionsToRemove.push(ExtractElementI);
      ...
      InstructionsToRemove.push(BinOpI);
    } else if (auto *CastI = dyn_cast<CastInst>(I)) {
      auto *SrcVal = CastI->getOperand(0);
      unsigned PaddedLength = GetMaxPaddedLength({CastI, SrcVal});
      ...
      InstructionsToRemove.push(CastI);
    } else if (auto *GEPI = dyn_cast<GetElementPtrInst>(I)) {
      ...
      InstructionsToRemove.push(GEPI);
    } else if (auto *LoadI = dyn_cast<LoadInst>(I)) {
      ...
      InstructionsToRemove.push(LoadI);
    } else if (auto *StoreI = dyn_cast<StoreInst>(I)) {
      ...
      if (IntrnscInst->getIntrinsicID() == Intrinsic::masked_load) {
        ...
        InstToPadded.insert_or_assign({IntrnscInst, PaddedLength},
                                      NewMaskedLoad);
        InstructionsToRemove.push(IntrnscInst);
      ...
      InstToPadded.insert_or_assign({InsrtElmntI, PaddedLength},
                                    NewInsrtElmntI);
      InstructionsToRemove.push(InsrtElmntI);
    } else if (auto *CmpI = dyn_cast<CmpInst>(I)) {
      ...
      InstructionsToRemove.push(CmpI);
    } else if (auto *SelectI = dyn_cast<SelectInst>(I)) {
      ...
      InstructionsToRemove.push(SelectI);
    } else if (auto *UnOpI = dyn_cast<UnaryOperator>(I)) {
      ...
      InstructionsToRemove.push(UnOpI);
    } else if (auto *ShflVctrI = dyn_cast<ShuffleVectorInst>(I)) {
      ...
      InstructionsToRemove.push(ShflVctrI);
    } else if (auto *Phi = dyn_cast<PHINode>(I)) {
  ...
  while (!InstructionsToRemove.empty()) {
    auto *I = InstructionsToRemove.top();
    InstructionsToRemove.pop();
    invalidateRippleDataFor(I);
...
void Ripple::genVectorInstructions() {
  ...
  auto processRippleSlice = [&](IntrinsicInst *RippleSlice,
    ...
    SmallVector<int64_t, N_RIPPLE_SLICE_ARGS> SliceArgs;
    llvm::transform(
    ...
    Value *SliceInst;
    if (ToShape.isScalar()) {
      SmallVector<size_t, N_RIPPLE_SLICE_ARGS> SliceIndex;
  ...
  while (!WorkList.empty()) {
    ...
    for (auto &I : make_early_inc_range(*BBToProcess)) {
      ...
      if (LoadInst *Load = dyn_cast<LoadInst>(&I)) {
        ...
        Value *OldMask = MaskedLoad->getArgOperand(2);
        ...
        MaskedLoad->setArgOperand(2, NewMask);
      } else if (IntrinsicInst *MaskedGather =
                     intrinsicWithId(&I, {Intrinsic::masked_gather})) {
        // Add the branch mask to the existing one
        Value *OldMask = MaskedGather->getArgOperand(2);
        Value *NewMask = irBuilder.CreateAnd(OldMask, MaskToApply,
                                             Twine(OldMask->getName()) +
                                                 ".ripple.branch.mask.apply");
        setRippleShape(NewMask, InstructionShape);
        MaskedGather->setArgOperand(2, NewMask);
        ...
        // Add the branch mask to the existing one
        Value *OldMask = MaskedStore->getArgOperand(3);
        Value *NewMask = irBuilder.CreateAnd(OldMask, MaskToApply,
                                             Twine(OldMask->getName()) +
                                                 ".ripple.branch.mask.apply");
        setRippleShape(NewMask, InstructionShape);
        MaskedStore->setArgOperand(3, NewMask);
      } else if (IntrinsicInst *MaskedScatter =
                     intrinsicWithId(&I, {Intrinsic::masked_scatter})) {
        // Add the branch mask to the existing one
        Value *OldMask = MaskedScatter->getArgOperand(3);
        Value *NewMask = irBuilder.CreateAnd(OldMask, MaskToApply,
                                             Twine(OldMask->getName()) +
                                                 ".ripple.branch.mask.apply");
        setRippleShape(NewMask, InstructionShape);
        MaskedScatter->setArgOperand(3, NewMask);
...
void Ripple::ifConvert() {
  ...
  for (auto *BranchingBB : BBsWithVectorBranchOrSwitch) {
    ...
    if (hasTrivialLoopLikeBackEdge(BranchingBB, BranchPostDom, domTree)) {
      dbgs() << "Yes there is a loop with trivial back-edge. This would lead to vectorial phis.\n";
...
bool Ripple::canBeReplacedAndLinearSeries(const Instruction *I) {
  // For now only ripple.block.index and ripple.block.getsize have this
  // property since the instruction will be replaced by the value 0, 1 and are
  // linear series.
  return intrinsicWithId(
      I, {Intrinsic::ripple_block_index, Intrinsic::ripple_block_getsize});
}
  ...
  for (auto *BB : BBsInBetween) {
    for (auto &I : *BB) {
      ...
      if (auto *Call = dyn_cast<CallInst>(&I)) {
        ...
        CheckInstruction = SelectToMaskWhenIfConvert.contains(Select);
      } else if (!ToSkipMaskingWhenIfConvert.contains(&I)) {
