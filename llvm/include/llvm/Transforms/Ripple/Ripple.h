...
public:
  ...
  /// @brief Returns true if a function is marked for Ripple Specialization
  static bool hasRippleErrors(const Function &F);
...
private:
  ...
  /// The set of call sites that require masking
  DenseSet<AssertingVH<CallInst>> FunctionCallThatNeedsMasking;
  ...
  /// @brief Returns the size of the PE block, defaults to 1 for everything
  /// that's not a vector.
  /// @param idAndIdx the dimension identifier and index
  /// @return the size of the dimension, or 1 if unknown
  static DimSize
  blockSize(RippleIntrinsicIndex idAndIdx,
            std::map<PEIdentifier, TensorShape::Shape> &vectorShapes);
  ...
  /// @brief Returns true if the instruction can be replaced *and* have a valid
  /// linear series associated with it at the same time.
  static bool canBeReplacedAndLinearSeries(const Instruction *I);
  ...
  /// @brief Checks if no vector dimensions have been defined
  ...
  /// @brief Updates the shape of instructions that are affected by a vector
  /// branch, i.e., StoreInst in the region under the vector branch and and PHIs
  /// in the post-dominator.
  /// @param BranchOrSwitch A BranchInst or SwitchInst
  /// @param BranchShape The shape of the branch/switch instruction
  /// @param  RecordChange A function that will be called on every Instruction
  /// whose shape has been updated by the vector branch
  Error promoteInstructionsAffectedByVectorBranch(
      Instruction *BranchOrSwitch, const TensorShape &BranchShape,
      std::function<void(Instruction *)> RecordChange);
