...
void memory_to_memory_scatter(float *In[32], float *Out[32][32]) {
  ...
  // CHECK: [[InGather:%.*]] = tail call <32 x float> @llvm.masked.gather.v32f32.v32p0(<32 x ptr> [[GatherAddrs]], i{{[0-9]+}} 4, <32 x i1> splat (i1 true), <32 x float> poison)
  ...
  for (unsigned i = 0; i < 32; ++i) {
    ...
    // CHECK: tail call void @llvm.masked.scatter.v32f32.v32p0(<32 x float> [[InGather]], <32 x ptr> [[ScatterAddrs]], i32 4, <32 x i1> splat (i1 true))
